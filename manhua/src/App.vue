  ...
  <div id="app">
    ...
    <div id="nav" class="f-flex1">
      <router-link to="/" class="f-flex2">
        ...
        <img class="animate__animated animate__rubberBand" src="./assets/tabBar/home2.png" alt="">
        首页
      </router-link>
      <router-link to="/new" class="f-flex2">
        <img src="./assets/tabBar/update1.png" alt="">
        <img class="animate__animated animate__bounce" src="./assets/tabBar/update2.png" alt="">
        更新
      </router-link>
      <router-link to="/collect" class="f-flex2">
        <img src="./assets/tabBar/collect1.png" alt="">
        <img class="animate__animated animate__swing" src="./assets/tabBar/collect2.png" alt="">
        收藏
      </router-link>
      <router-link to="/my" class="f-flex2">
        <img src="./assets/tabBar/my1.png" alt="">
        <img class="animate__animated animate__wobble" src="./assets/tabBar/my2.png" alt="">
        我的
      </router-link>
    </div>
    <van-popup class="watchP" position="right" v-model="watchPShow">
      <mt-loadmore :top-method="jiazaiLeft" @top-status-change="handleTopChange" :bottom-method="jiazai"
                   @bottom-status-change="handleBottomChange" :auto-fill="false" ref="loadmore2">
        <main>
          <div v-for="(val,index) in book.slice(start,more)" :key="index" @click="conf(val)">
            <img v-for="i in val.end_num" v-lazy="where(val,i)" alt="">
          </div>
        </main>
        <div slot="top" class="mint-loadmore-top">
          <span v-show="topStatus !== 'loading'" :class="{ 'rotate': topStatus === 'drop' }">释放进入上一章</span>
          <span v-show="topStatus === 'loading'">
						<van-loading size="24px">loading...</van-loading>
					</span>
        </div>
        <div slot="bottom" class="mint-loadmore-bottom">
          <span v-show="bottomStatus !== 'loading'" :class="{ 'rotate': bottomStatus === 'drop' }">释放进入下一章</span>
    ...
    <van-popup :overlay="false" class="to f-flex1" position="top" v-model="backShow">
    ...
    <van-popup :overlay="false" class="bo f-flex1" position="bottom" v-model="backShow">
      <img src="./assets/img/left1.png" @click="left" v-show="curNum>0" alt="">
      <img src="./assets/img/left2.png" @click="tip('前面没有了哦')" v-show="curNum===0" alt="">
      <span @click="catalogueList">目录</span>
      <img src="./assets/img/right1.png" v-show="more<book.length" @click="right" alt="">
      <img src="./assets/img/right2.png" @click="tip('后面没有了哦')" v-show="more===book.length" alt="">
      <div class="f-flex3">
        <img @click="changeLove(bookInfo)" src="./assets/img/love1.png" alt="" v-show="!checkLove">
        <img @click="changeLove(bookInfo)" src="./assets/img/love2.png" alt="" v-show="checkLove">
      </div>
    </van-popup>
    <van-popup :overlay="false" class="catalogueP" position="right" v-model="catalogueShow">
      <main ref="cata" style="height: 100%;overflow: scroll">
        <div @click="change(index)" v-for="(val,index) in book" :key="index"
...
export default {
  ...
  methods: {
    ...
    watchP(id) {
      ...
      let url = 'http://comic.65sy.com/app_api/v5/getcomicinfo_body/?client-version=&comic_id=' + id +
  ...
  computed: {
    ...
    ...mapState(['collectBooks', 'historyBooks'])
  },
  watch: {
    collectBooks: {
      deep: true,
